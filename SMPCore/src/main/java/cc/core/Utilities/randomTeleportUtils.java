...
public class randomTeleportUtils {
    ...
    public static Location generateLocation(Player player){
        ...
        if(plugin.getConfig().getBoolean("rtp-radius-limit")){ //If they want to limit the distance
            x = random.nextInt(plugin.getConfig().getInt("rtp-radius")) + 250;
            z = random.nextInt(plugin.getConfig().getInt("rtp-radius")) + 250;
            y = 150;
        }else if(!plugin.getConfig().getBoolean("rtp-radius-limit")){ //If they don't
            x = random.nextInt(25000) + 250; //25000 is default
            z = random.nextInt(25000) + 250;
            y = 150;
        }
