...
import core.server.core.API.SetFlySpeed;
        ...
        if (cmd.getName().equalsIgnoreCase("fly")) {
            ...
            if (args.length == 0) {
                if (!(sender instanceof Player)) {
                    ...
                    if (!(sender.hasPermission("rank.staff"))) {
                        ...
                        if (Boolean.TRUE.equals(p.getAllowFlight())) {
                            ...
                            p.sendMessage(Core.Plname+"Flight has been "+ChatColor.RED+"disabled"+ChatColor.GRAY+".");
                            return true;
                        }
                        if (Boolean.FALSE.equals(p.getAllowFlight())) {
                            p.setAllowFlight(true);
                            p.sendMessage(Core.Plname+"Flight has been "+ChatColor.GREEN+"enabled"+ChatColor.GRAY+".");
                            return true;
                        }
                    }
                }
                ...
                if (!(sender.hasPermission("rank.admin"))) {
                    ...
                    Player target = getServer().getPlayer(args[0]);
                    if (target == null){
                        sender.sendMessage(Core.Plname+ChatColor.RED+"Player offline");
                        return true;
                    }
                    if (Boolean.TRUE.equals(target.getAllowFlight())) {
                        target.setAllowFlight(false);
                ...
                if (!(sender.hasPermission("rank.admin"))) {
                    ...
                    sender.sendMessage(Core.Plname+ ChatColor.RED + "Invalid usage pick between 1-10 for fly speed.");
                }
            } else {
