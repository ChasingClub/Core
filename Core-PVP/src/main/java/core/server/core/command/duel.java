...
public class duel implements CommandExecutor, TabCompleter {
    ...
    public String usage = Core.Plname + ChatColor.YELLOW + "/duel <invite/accept/reject> <player>";
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) { // player
            ...
            if (args.length == 0) {
                ...
                player.sendMessage(Core.Plname + ChatColor.RED + "You need to provide an options to duels");
                ...
                if (args[0].equalsIgnoreCase("leave")){
                    if (Core.ingame.get(player.getName()) == null) {
                        player.sendMessage(Core.Plname+ChatColor.RED+"You are not in the game!");
                        return true;
                    }
                    ...
                    Core.combatList.put(player.getName(), 0);
                    Core.combatList.put(target.getName(), 0);
                    target.teleport(Spawn);
                    target.sendMessage(Core.Plname + "GG! you have defeated " + player.getName());
                    player.teleport(Spawn);
                    player.sendMessage(Core.Plname + "You lost because you left the game!");
                ...
                if (Core.duel.get(args[0]) != null) {
                    ...
                    player.sendMessage(Core.Plname + ChatColor.RED + "You need to specify a player to duels!");
                    player.sendMessage(Core.Plname + ChatColor.YELLOW + "/duel "+args[0]+" <player>");
                ...
                if (args[0].equalsIgnoreCase("invite")) {
                    ...
                    if (target == null) {
                        ...
                        player.sendMessage(Core.Plname + ChatColor.RED + "That player is offline!");
                        return true;

                    }if (target.getName().equals(player.getName())) {

                        player.sendMessage(Core.Plname + ChatColor.RED + "You can't duel yourself!");
                        return true;

                    } else { // not specify game

                        player.sendMessage(Core.Plname + "You need to specify the game!");
                        player.sendMessage(Core.Plname + game);
                    ...
                    if (Core.inviteList.get(player.getName()) == null) { // if target doest invite the player
                        ...
                        player.sendMessage(Core.Plname + target.getName() + " doesn't invite you to duel!");
                        return true;
                    ...
                    if (Core.inviteList.get(player.getName()) != null) {
                        ...
                        if (Core.inviteList.get(player.getName()).containsValue("netheritestack")) {
                            ...
                            if (Boolean.TRUE.equals(Core.maps.get("Colosseum"))) {
                                target.sendMessage(Core.Plname + player.getName() + ChatColor.GREEN + " has accept the duel"); // accept
                                ...
                                player.sendMessage(Core.Plname+"You are currently playing on "+ChatColor.GREEN+"\"Colosseum\"");
                                target.sendMessage(Core.Plname+"You are currently playing on "+ChatColor.GREEN+"\"Colosseum\"");
                                ...
                                player.setGameMode(GameMode.ADVENTURE);
                                target.setGameMode(GameMode.ADVENTURE);
                                player.sendMessage(Core.Plname+"You are currently playing on "+ChatColor.GREEN+"\"Beach\"");
                                target.sendMessage(Core.Plname+"You are currently playing on "+ChatColor.GREEN+"\"Beach\"");
                                Core.ingame.put(player.getName(), target.getName());
                                Core.ingame.put(target.getName(), player.getName());
                                ...
                                player.setGameMode(GameMode.ADVENTURE);
                                target.setGameMode(GameMode.ADVENTURE);
                                player.sendMessage(Core.Plname+"You are currently playing on "+ChatColor.GREEN+"\"Abyss\"");
                                target.sendMessage(Core.Plname+"You are currently playing on "+ChatColor.GREEN+"\"Abyss\"");
                                Core.ingame.put(player.getName(), target.getName());
                                Core.ingame.put(target.getName(), player.getName());
                                ...
                                player.sendMessage(Core.Plname+ChatColor.RED+"No area available.");
                                Core.inviteList.remove(player.getName());
                                return true;
                            }
                        } else {
                            player.sendMessage(Core.Plname + ChatColor.RED + "Game: " + game);
                        }
                    }
                    ...
                    if (!(Core.inviteList.containsKey(player.getName()))) {
                        ...
                        player.sendMessage(Core.Plname + target.getName() +ChatColor.GREEN+ " has reject the duels"); // reject
                        Core.inviteList.remove(player.getName());
                        return true;
                    }
                ...
                if (Core.games.get(args[2].toLowerCase()) == null) {
                    ...
                    if (args[0].equalsIgnoreCase("invite")) {
                        ...
                        if (target == null) {
                            ...
                            player.sendMessage(Core.Plname + ChatColor.RED + "That player is currently in a game!");
                            return true;

                        }if (Core.inviteList.get(target.getName()) != null){

                            player.sendMessage(Core.Plname + ChatColor.RED + "You already invite that player!");
                            return true;

                        } else {

                            // send to player with usage
                            target.sendMessage(Core.Plname + ChatColor.GREEN + " " + playerName + " invites you for duels");
                            target.sendMessage(Core.Plname + ChatColor.GREEN + " invite will be remove in 20s");
                            target.sendMessage(Core.Plname + ChatColor.GREEN + "Use /duel <accept/reject> " + playerName);
                            ...
                            player.sendMessage(Core.Plname+"Duel have been sent to "+target.getName());
                        ...
                        player.sendMessage(usage);
                        player.sendMessage(Core.Plname + "Game -> " + game);
                    }
